...
import { MergeTreeDeltaType, PropertySet } from "@fluidframework/merge-tree";
...
export interface AddText extends Rollbackable {
	type: "addText";
	index: number;
	content: string;
}

export interface RemoveRange extends RangeSpec, Rollbackable {
	type: "removeRange";
}

// For non-interval collection fuzzing, annotating text would also be useful.
export interface AddInterval extends IntervalCollectionSpec, RangeSpec, Rollbackable {
	type: "addInterval";
...
export interface ChangeInterval extends IntervalCollectionSpec, RangeSpec, Rollbackable {
	type: "changeInterval";
	id: string;
	startSide: Side;
	endSide: Side;
...
export interface DeleteInterval extends IntervalCollectionSpec, Rollbackable {
	type: "deleteInterval";
	id: string;
}

export interface ChangeProperties extends IntervalCollectionSpec, Rollbackable {
...
export type Operation = RevertOperation; // | RollbackOperation;
...
export interface SharedStringOperationGenerationConfig {
	...
	/**
	 * If specified, the probablility of rolling back a given op.
	 *
	 * Default value: 0.0
	 */
...
export function makeReducer(
	...
	const reducer = combineReducers<Operation | RevertOperation, ClientOpState>({
		addText: async ({ client }, { index, content, rollback }) => {
			client.channel.insertText(index, content);

			if (rollback) {
				// eslint-disable-next-line @typescript-eslint/dot-notation
				client.channel["rollback"](
					{ type: MergeTreeDeltaType.INSERT },
					// eslint-disable-next-line @typescript-eslint/dot-notation
					client.channel["client"].peekPendingSegmentGroups(),
				);
			}
		},
		removeRange: async ({ client }, { start, end, rollback }) => {
...
export function createSharedStringGeneratorOperations(
	...
	function rollback(state: ClientOpState) {
		if (options.rollbackProbability <= 0) {
			return undefined;
		}

		return { rollback: state.random.bool(options.rollbackProbability) };
	}
	...
	async function removeRange(state: ClientOpState): Promise<RemoveRange> {
		return { type: "removeRange", ...exclusiveRange(state), ...rollback(state) };
	}

	async function removeRangeLeaveChar(state: ClientOpState): Promise<RemoveRange> {
		return { type: "removeRange", ...exclusiveRangeLeaveChar(state), ...rollback(state) };
	}
